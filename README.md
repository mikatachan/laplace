...
**Priority-aware admission control + idle and external-tenant model reaping for a *shared* local LLM inference pool.**
...
> Status: **early / extracting.** The design runs in production (a multi-agent Discord bot on an M4 Ultra + LM Studio). This repo is the standalone, engine-adapter extraction of that core. The LM Studio adapter is the reference; Ollama/vLLM adapters are the obvious next contributions.

---

## What's actually new here

A [prior-art scan](#prior-art--where-laplace-fits) found that the *basic* pieces — an admission cap + idle TTL for models **you** loaded — are commodity (LM Studio's own TTL/Auto-Evict, Ollama `keep_alive`, llama-swap, LocalAI, llama.cpp router). What no surveyed tool does:
...
If you don't have other processes sharing your server and you just want idle-unload, your runtime already does that — you may not need this.

---

## Core (engine-agnostic) + adapters
...
## Limitations
- Leans on the runtime's activity-signal schema (`lms ps --json` fields) — undocumented, could drift; fails safe (unknown → don't reap).
